...
export default async function ServicoDetalhePage({ params }: Props) {
 ...
 <div className="text-xs text-fg-subtle">{servico.cliente?.telefone || "—"}</div>
 </div>
 <div className="h-px bg-border" />
 <div>
 <div className="text-xs text-fg-subtle">Diarista</div>
 <div className="font-semibold">{servico.diarista?.nome || "—"}</div>
 <div className="text-xs text-fg-subtle">{servico.diarista?.telefone || "—"}</div>
 </div>
 </div>
 </AdminCard>
 </div>
 ...
 <li key={ev.id} className="rounded-xl border border-border bg-surface-secondary px-3 py-2 ">
 ...
 <div className="text-xs text-fg-subtle">{fmtDate(ev.createdAt)}</div>
 </div>
 </li>
 ))}
 </ol>
 ...
 <label className="text-xs text-fg-subtle">Motivo do cancelamento</label>
 <textarea
 name="motivo"
 ...
 <input type="hidden" name="id" value={servico.id} />
 <label className="text-xs text-fg-subtle">Marcar como disputa (motivo)</label>
 <textarea
 name="motivo"
 className="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-fg outline-none focus:ring-2 focus:ring-accent/30"
