...
function EmptyState({ accentColor, softBg }: { accentColor: string; softBg: string }) {
  return (
    <View style={styles.emptyWrap}>
      ...
      <Text style={styles.emptySubtitle}>Seus chats com clientes aparecem aqui</Text>
    </View>
  );
}
...
export function MontadorMensagens() {
  ...
  const renderItem = useCallback(
    ({ item }: { item: ChatRoom }) => (
      <ConversaCard room={item} onPress={() => navigation.navigate("MontadorChat", { servicoId: item.servicoId })} />
    ),
    [navigation],
  );
